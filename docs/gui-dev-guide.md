...
- [ ] **前端 SVG 彩色曲线图**:用 `log_topology` 画 node + lane 配色 + edge 曲线,替换 History 等宽 ASCII 图。

**MVP 之后的后续工作统一收在 [gui-backlog.md](gui-backlog.md)**(可直接派发的任务卡):前端曲线图、抽 DiffView 共享组件、文件 watch 自动刷新、行内字符级 diff、Windows 适配、跨平台 CI + 签名、plan 流式化。
