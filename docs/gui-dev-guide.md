...
git-gui/
  ...
  gui/
    ...
    src/
      lib/
        DiffView.svelte   共享 diff 组件(Changes 交互/History 只读)
        HistoryView.svelte
        UpdateView.svelte
      routes/+page.svelte 前端入口(Changes 视图 + tab 切换)
      routes/+layout.ts   ssr=false,纯客户端 SPA
    src-tauri/
      src/lib.rs          命令定义 + Tauri Builder(run())
      src/main.rs         入口(调 gui_lib::run())
      Cargo.toml          后端依赖(tauri + gitcore[features=serde] + notify)
...
3. ✅ **spawn git 加 no-window flag**:`git_command()` helper,所有 `Command::new("git")` 在 Windows 自动注入 `CREATE_NO_WINDOW`。
4. ✅ **git 存在性检测**:`Repo::check_git()` 静态方法,前端可调用 `check_git` 命令。

构建分发:跨平台用 CI(GitHub Actions 各平台 runner)各编各的。签名/公证(macOS notarization、Windows 签名)后置。
...
|---|---|---|
| **Changes** | `unstaged_diff`/`staged_diff`(文件→hunk→行)、`stage`/`unstage`/`stage_hunk`/`stage_lines`、`commit` | ✅ |
| **History** | `log_topology` → `GraphCommit{lane,edges}` + SVG 彩色曲线图、`commit_files`、`commit_message` | ✅ |
...
- [x] **gitcore 结构化 log 拓扑**(`topology.rs`:lane/edge 算法 + 9 单测,PR #22 已合并)——彩色曲线图的后端基础。
- [x] **前端 SVG 彩色曲线图**:用 `log_topology` 画 node + lane 配色 + edge 曲线,替换 History 等宽 ASCII 图。
- [x] **DiffView 共享组件**:Changes/History 的 structured diff 渲染收敛到 `DiffView.svelte`,支持只读+可交互两态。
- [x] **文件 watch 自动刷新**:外部改动(Git/IDE)后 ~300ms 自动刷新 GUI 视图(`notify` crate + debounce emit)。
- [x] **行内字符级 diff**:配对修改行做 LCS 字符级高亮,改动收敛在 DiffView,零新依赖。
- [x] **Windows 适配**:`CREATE_NO_WINDOW` 防黑框 + git 存在性检测。
- [x] **跨平台 CI**:GitHub Actions 三平台(ubuntu/macos/windows)构建 + lint/test 质量门(签名/公证后置)。
- [x] **plan 流式化**:`plan_update` 的 fetch 阶段已有进度条 + 可取消,修了取消竞态 bug。
...
- **lint-test** — `cargo clippy`（gitcore + gui + tui）+ `cargo fmt --check` + `cargo test -p gitcore`
...
Rust 依赖缓存 key 基于 `Cargo.lock` hash，未命中时从最近 key 恢复。npm 缓存基于 `gui/package-lock.json`。
...
- CI 全绿（lint + test + svelte-check + 三平台构建）
